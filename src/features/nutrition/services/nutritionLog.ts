...
import type { Food, MacroBreakdown, MealType } from '@/types'
...
export async function logNewFood(input: NewFoodLogInput): Promise<void> {
  ...
  const existing = store.foods.find(
...
export async function relogFood(food: Food, mealType: MealType): Promise<void> {
  const now = Date.now()
  const store = useDataStore.getState()
  store.updateFood(food.id, { lastLoggedAt: now, timesLogged: food.timesLogged + 1 })
  store.addFoodLog({
    id: createId(),
    dateKey: todayKey(),
    mealType,
...
export async function deleteFoodLogEntry(entryId: string): Promise<void> {
  useDataStore.getState().deleteFoodLog(entryId)
}

export async function logWater(amountMl: number): Promise<void> {
  useDataStore.getState().addWaterLog({
    id: createId(),
    dateKey: todayKey(),
    amountMl,
    loggedAt: Date.now(),
